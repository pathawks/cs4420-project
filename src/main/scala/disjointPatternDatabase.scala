...
class disjointPatternDatabase {
 ...
 def getDPDB(initial:State, makeNodes: (State,List[Int],List[Int]) =>List[(State,Operator,Boolean,Boolean)],maxCost:(Int,Int,Int,Int), mode:Int):Unit = {
  ...
  if (mode==0|mode==2){
    if (s==3) {left=List(1,4,7,8)}
    else {left= List(1,2,5,6,9,10,13,14)}}
  if (mode==1|mode==2){
    if (s==3) {up=List(1,2,3,4)}
    else { up= List(1,2,3,4,5,6,7,8)}}
  println(s)
  def search(a:State, g:(Int,Int,Int,Int),mode:Int):Unit = { 
    if (mode==0 |mode==2){
    val (left,right)=patternDPDB(a, true)
    if (!( lpdb contains left))  lpdb= lpdb +(left->g._3)
    else { if (lpdb(left)>g._1) lpdb= lpdb +(left->g._3)}
    if (!( rpdb contains right))  rpdb= rpdb +(right->g._4)
    else { if (rpdb(right)>g._2) rpdb= rpdb +(right->g._4)}}

    if (mode==1 | mode==2){
    val (up,down)=patternDPDB(a, false)
    if (!( updb contains up))  updb= updb +(up->g._1)
    else { if (updb(up)>g._1) updb= updb +(up->g._1)}
    if (!( dpdb contains down)) dpdb= dpdb +(down->g._2)
    else { if (dpdb(down)>g._2) dpdb= dpdb +(down->g._2)}}
...
def disjointPDB(s:State, mode:Int, move:Int):Int={
  ...
  if ((mode==0 & (lpdb.size==0|rpdb.size==0)) | (mode==1 & (updb.size==0|dpdb.size==0))) 
    {println("____");getDPDB(goalState(size),validMovesDPDB,(move,move,move,move),mode)}
